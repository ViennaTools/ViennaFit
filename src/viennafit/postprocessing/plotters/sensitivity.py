...
class SensitivityPlotter(BasePlotter):
    ...
    def plot(self, data: StudyData, outputDir: str) -> List[str]:
        ...
        if "firstOrder" in sensitivityResults:
            filepath = self._plotFirstOrderIndices(data, sensitivityResults, outputDir)
            if filepath:
                createdFiles.append(filepath)

        # Total-order sensitivity indices
        if "totalOrder" in sensitivityResults:
            filepath = self._plotTotalOrderIndices(data, sensitivityResults, outputDir)
            if filepath:
                createdFiles.append(filepath)

        # Combined comparison of first and total order
        if "firstOrder" in sensitivityResults and "totalOrder" in sensitivityResults:
        ...
        if "secondOrder" in sensitivityResults:
            filepath = self._plotSecondOrderHeatmap(data, sensitivityResults, outputDir)
            if filepath:
                createdFiles.append(filepath)

        return createdFiles

    def _plotFirstOrderIndices(
        self, data: StudyData, sensitivityResults: Dict, outputDir: str
    ) -> str:
        ...
        sortedParams = sorted(firstOrder.items(), key=lambda x: abs(x[1]), reverse=True)
    ...
    def _plotTotalOrderIndices(
        ...
        # Sort parameters by sensitivity value
        sortedParams = sorted(totalOrder.items(), key=lambda x: abs(x[1]), reverse=True)
        paramNames = [p[0] for p in sortedParams]
        values = [p[1] for p in sortedParams]
    ...
    def _plotSensitivityComparison(
        self, data: StudyData, sensitivityResults: Dict, outputDir: str
    ) -> str:
        ...
        sortedParams = sorted(
            commonParams, key=lambda x: abs(totalOrder[x]), reverse=True
        )
