...
class ResultsLoader:
    ...
    def loadStudyData(self) -> StudyData:
        ...
        data = StudyData(
            runDir=self.runDir, studyName=self.studyName, studyType=studyType
        )
    ...
    def _detectStudyType(self) -> str:
        ...
        if (
            os.path.exists(os.path.join(self.runDir, "sensitivity_results.json"))
            or os.path.exists(os.path.join(self.runDir, "parameter_samples.csv"))
            or os.path.exists(os.path.join(self.runDir, "evaluations"))
        ):
        ...
        csvPaths = [
            ...
            (
                ...
                [
                    ...
                    os.path.join(
                        self.runDir, "progressAll_best.csv"
            ...
            for path in pathCandidates:
                if os.path.exists(path):
                    try:
                        ...
                        print(
                            f"Warning: Could not load CSV progress data from {path}: {e}"
                        )
            ...
            for dataType, path in txtPaths:
                if os.path.exists(path):
                    try:
                        ...
                        print(
                            f"Warning: Could not load TXT progress data from {path}: {e}"
                        )
